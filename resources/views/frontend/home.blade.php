@extends('frontend.layouts.master')

@section('content')
    <main>
        <section class="my-5">
            <div class="container">
                <div class="row">
                    <div class="col-md-8">
                        <div class="slider-index shadow-sm">
                            <div id="carouselExampleControls" class="carousel slide" data-bs-ride="carousel">
                                <div class="carousel-inner">
                                    <div class="carousel-item active">
                                        <img src="{{asset('frontend/img/slider/Woo_Package_New_Bnr1.png')}}" class="d-block w-100 rounded" alt="">
                                    </div>
                                    <div class="carousel-item">
                                        <img src="{{asset('frontend/img/slider/Woo_Package_New_Bnr2.png')}}" class="d-block w-100 rounded" alt="">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-4">
                                <div class="b-slider-btn">
                                    <a href="#" class="bg-primary rounded d-block p-3 shadow-sm text-center my-1 my-md-3 text-light"><i class="bi bi-filter-left"></i> کتاب و رمان</a>
                                </div>
                            </div>
                            <div class="col-sm-4">
                                <div class="b-slider-btn">
                                    <a href="#" class="bg-primary rounded d-block p-3 shadow-sm text-center my-1 my-md-3  text-light"><i class="bi bi-person"></i> پیوستن به انجمن</a>
                                </div>
                            </div>
                            <div class="col-sm-4">
                                <div class="b-slider-btn">
                                    <a href="#" class="bg-warning rounded d-block p-3 shadow-sm text-center my-1 my-md-3  text-dark"><i class="bi bi-gem"></i> خرید عضویت ویژه</a>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="search-box">
                                    <form action="" class="position-relative w-100 mb-3 mb-md-0">
                                        <input type="text" placeholder="جستجو کنید ..." class="pe-3">
                                        <button class="position-absolute bg-dark"><i class="bi bi-search"></i></button>
                                    </form>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4 top-slider">
                        <div class="slider-item bg-white rounded p-4">
                            <figure class="position-relative">
                                <img src="{{asset('frontend/img/blog/youtube.jpg')}}" alt="" class="w-100 rounded">
                                <figcaption class="tp_top position-absolute p-4">
                                    <span>20 مرداد</span>
                                </figcaption>
                                <figcaption class="tp_bottom position-absolute p-4">
                                    <i class="bi bi-play bg-white d-inline-block text-center"></i>
                                </figcaption>
                            </figure>
                            <div class="item-cat">
                                <div class="d-flex">
                                    <i class="bi bi-play-circle"></i>
                                    <span class="d-block p-2 text-info">بروز شده</span>
                                </div>
                            </div>
                            <a href="#">
                                <h6 class="overflow-hidden px-2">تاثیر ویدئو در سئو</h6>
                            </a>
                            <a href="#" class="d-block bg-dark p-3 mt-3 text-center text-light rounded">مطالب ویژه اعضای Vip</a>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <section class="my-3">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="title-box mb-3">
                            <p class="m-0 p-0">تازه ترین ها</p>
                            <span class="d-block"><a href="#">مشاهده همه</a> <i class="bi bi-arrow-left position-relative"></i></span>
                        </div>
                    </div>
                    <div class="col-12">
                        <div class="top-slider owl-carousel owl-theme top-slider-wrap">
                            @foreach($last_articles as $article)
                                <div class="slider-item bg-white rounded p-4">
                                    <figure class="position-relative">
                                        <img src="{{url('images/articles/'. $article->image)}}" alt="عکس بارگزاری نشد" class="w-100 rounded">
                                        <figcaption class="tp_top position-absolute p-4">
                                            <span>{{Hekmatinasser\Verta\Verta::instance($article->created_at)->format('%d %B')}}</span>
                                        </figcaption>
                                        <figcaption class="tp_bottom position-absolute p-4">
                                            <i class="bi bi-play bg-white d-inline-block text-center"></i>
                                        </figcaption>
                                    </figure>
                                    <a href="{{route('article',$article->id)}}">
                                        <h6 class="overflow-hidden px-2">{{$article->title}}</h6>
                                    </a>
                                </div>
                            @endforeach
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <section>
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-12">
                        <div class="title-box mb-3">
                            <p class="m-0 p-0">بیشترین بازدید ها</p>
                            <span class="d-block"><a href="#">مشاهده همه</a> <i class="bi bi-arrow-left position-relative"></i></span>
                        </div>
                    </div>
                    @foreach($mostViewed_articles as $article)
                        <div class="col-sm-6 col-md-6 col-lg-4 my-3">
                            <div class="blg-wrap bg-white pb-3">
                                <figure class="position-relative">
                                    <img src="{{url('images/articles/'.$article->image)}}" alt="تصویر بارگذاری نشد" class="w-100">
                                    <figcaption class="position-absolute w-100 h-100"><span class="blg-dt">{{Hekmatinasser\Verta\Verta::instance($article->created_at)->format('%d %B')}}</span></figcaption>
                                </figure>
                                <div class="px-3 py-1 text-start">
                                    <span class="blg-cat-name d-inline-block bg-info py-1 px-2">{{$article->category->title}}</span>
                                </div>
                                <a href="{{route('article',$article->id)}}">
                                    <h4 class="blg-title p-3 mx-5 text-dark border-bottom overflow-hidden">{{$article->title}}</h4>
                                </a>
                                <p class="p-4 overflow-hidden">{{mb_substr($article->body,0,200)}}...</p>
                            </div>
                        </div>
                    @endforeach
                </div>
            </div>
        </section>
    </main>
@endsection
